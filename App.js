...
const StockManage = () => {
  ...
  // 覆盖：食品、耐用品、器械、鞋包、餐具、家电、文具、电子产品、母婴、日化等
  const suggestExpiryByProductName = (name) => {
    ...
    // 饮料 / 水 → 365天
    if (/饮料|可乐|雪碧|果汁|咖啡|茶|矿泉水|纯净水|桶装水|瓶装水/.test(n)) return 365;
    // 干货 / 调料 / 米面 → 365天
    if (/米|面|粉|调料|酱油|醋|盐|糖|干货|木耳|香菇|红枣|枸杞|桂圆|花椒|八角|大料|粉丝|挂面|年糕/.test(n)) return 365;

    // ======== 中长期食品 ========
    // 保健品 / 营养品 → 540天
    if (/保健|营养|维生素|钙片|鱼油|蛋白粉|阿胶|人参|鹿茸|燕窝|口服液|胶囊|颗粒/.test(n)) return 540;
    // 药品 / 医疗用品 → 730天
    if (/药|片|胶囊|注射液|阿莫西林|头孢|布洛芬|感冒|止咳|创可贴|纱布|酒精棉球|体温计/.test(n)) return 730;

    // ======== 日化 / 清洁 ========
    // 日化 / 清洁 / 香薰 → 730-1095天
    if (/洗发|沐浴|香皂|牙膏|洗面奶|护手霜|润肤|保湿|面霜|化妆|口红|面膜|精华|水乳|护发|洗衣|洗洁精|洗衣液|消毒液|洁厕|抽纸|卷纸|纸巾|湿巾|垃圾袋|保鲜袋|保鲜膜|香薰|蜡烛/.test(n)) return 730;

    // ======== 母婴 ========
    if (/奶粉|奶瓶|纸尿裤|尿不湿|湿巾|婴儿|宝宝|辅食|儿童霜|儿童牙刷|学步车|推车/.test(n)) return 540;

    // ======== 耐用品（几乎无保质期，但给个"建议更换周期"） ========
    // 电子产品 → 1095天 (3年)
    if (/手机|平板|电脑|笔记本|显示器|键盘|鼠标|耳机|音箱|蓝牙|充电|电源|硬盘|U盘|内存|显卡|主板|路由器|相机|摄像机|无人机|投影仪|智能手表|手环|充电宝|数据线|摄像头|台灯|电池/.test(n)) return 1095;
    // 家电 → 1825-3650天 (5-10年)
    if (/冰箱|洗衣机|空调|电视|微波炉|烤箱|油烟机|燃气灶|热水器|吸尘器|扫地机|空气炸锅|电饭煲|压力锅|电磁炉|豆浆机|榨汁机|破壁机|净水器|加湿器|电风扇|暖风机|消毒柜|洗碗机|挂烫机|熨斗|电吹风|卷发棒|剃须刀|理发器/.test(n)) return 1825;
    // 餐具 / 厨具 → 3650天 (10年)
    if (/碗|盘|筷|勺|叉|刀|锅|杯|壶|盆|砧板|案板|厨具|餐具|不锈钢|陶瓷|玻璃碗|马克杯|保温杯|饭盒|保温桶|不粘锅|炒锅|汤锅|蒸锅|平底锅|高压锅|菜刀|水果刀/.test(n)) return 3650;
    // 文具 / 办公 → 3650天
    if (/笔|本|纸|墨|橡皮|尺子|胶带|胶水|订书|文件夹|文件袋|回形针|便签|便签纸|贴纸|画板|彩笔|蜡笔|水彩|油画棒|铅笔|圆珠笔|签字笔|钢笔|毛笔|笔记本|作业本|草稿|日历|台历|文件夹|档案盒|收纳/.test(n)) return 3650;
    // 家具 / 家居 → 7300天 (20年)
    if (/床|沙发|桌|椅|柜|衣柜|书柜|书桌|餐桌|茶几|电视柜|鞋柜|床头柜|床垫|床品|被套|枕套|窗帘|地毯|装饰画|相框|花瓶|花盆|绿植/.test(n)) return 7300;
    // 鞋包 / 服饰 → 1825天 (5年)
    if (/鞋|靴|帆布鞋|运动鞋|皮鞋|拖鞋|凉鞋|高跟鞋|包|背包|挎包|手提包|书包|钱包|腰包|行李箱|拉杆箱|羽绒服|外套|夹克|毛衣|卫衣|T恤|衬衫|裤|裙|袜|帽|围巾|手套|腰带|皮带|领带|领结|内衣|泳装/.test(n)) return 1825;
    // 玩具 / 儿童用品 → 3650天
    if (/玩具|积木|拼图|遥控车|玩偶|公仔|芭比|乐高|魔方|陀螺|泡泡枪|乐器|吉他|钢琴|电子琴|架子鼓|画板|蜡笔|彩泥|橡皮泥|超轻粘土/.test(n)) return 3650;
    // 运动 / 户外 → 1825天
    if (/球|球拍|篮球|足球|羽毛球|网球|乒乓球|瑜伽垫|哑铃|杠铃|跳绳|呼啦圈|拉力器|俯卧撑|仰卧板|跑步机|椭圆机|划船机|帐篷|睡袋|登山|徒步|滑雪|游泳圈|救生衣|自行车|平衡车|滑板|头盔|护膝/.test(n)) return 1825;
    // 五金 / 工具 / 建材 → 3650-7300天
    if (/扳手|螺丝刀|锤子|钳子|电钻|电锯|角磨机|电锤|卷尺|水平仪|游标卡尺|万用表|灯泡|LED|开关|插座|电线|水管|阀门|龙头|花洒|地漏|瓷砖|地砖|地板|木板|涂料|油漆|乳胶漆|胶水|免钉胶/.test(n)) return 3650;
    // 医疗器械 / 康复 → 3650天
    if (/血压计|血糖仪|体温计|电子秤|体重秤|轮椅|拐杖|助行器|按摩|理疗|牵引|护腰|护膝|护颈|疝气带|颈椎牵引/.test(n)) return 3650;
    // 农资 / 园艺 → 365-1095天
    if (/肥料|化肥|种子|农药|杀虫剂|除草剂|花盆|花架|喷壶|浇水壶|锄头|铁锹|耙子|镰刀|花卉|多肉|绿植/.test(n)) return 365;
    // 母婴耐用品 → 3650天
    if (/婴儿车|推车|学步车|餐椅|婴儿床|摇篮|安全座椅|奶瓶刷|奶瓶架|辅食机/.test(n)) return 3650;
    // 宠物用品 → 365-1825天
    if (/猫粮|狗粮|宠物粮|猫砂|狗绳|猫抓板|窝|喂水器|牵引|项圈|宠物玩具|驱虫/.test(n)) return 365;

    // 默认 → 90天 (AI 识别新商品时也会给出 expiryHint 覆盖)
    return 90;
  };

  // 判断一个品类是"耐用品"还是"食品/易耗"
  const isDurableCategory = (nameOrCat) => {
    if (!nameOrCat) return false;
    const n = nameOrCat;
    const durKeywords = /手机|电脑|平板|键盘|鼠标|耳机|音箱|充电|硬盘|U盘|相机|路由器|台灯|冰箱|洗衣机|空调|电视|微波炉|烤箱|油烟机|燃气灶|热水器|吸尘器|电饭煲|电磁炉|豆浆机|破壁机|净化器|加湿器|电扇|暖风机|消毒柜|洗碗机|挂烫|熨斗|电吹风|卷发|剃须刀|理发|碗|盘|筷|勺|叉|刀|锅|杯|壶|盆|砧板|厨具|餐具|不锈钢|陶瓷|玻璃|马克杯|保温杯|饭盒|保温桶|不粘锅|炒锅|汤锅|蒸锅|平底锅|高压锅|菜刀|水果刀|笔|本|纸|墨|橡皮|尺子|胶带|胶水|订书|文件夹|文件袋|回形针|便签|贴纸|画板|彩笔|蜡笔|水彩|油画棒|铅笔|圆珠笔|签字笔|钢笔|毛笔|笔记本|作业本|草稿|日历|台历|床|沙发|桌|椅|柜|衣柜|书柜|书桌|餐桌|茶几|电视柜|鞋柜|床头柜|床垫|床品|被套|枕套|窗帘|地毯|装饰画|相框|花瓶|花盆|绿植|鞋|靴|帆布鞋|运动鞋|皮鞋|拖鞋|凉鞋|高跟|包|背包|挎包|手提包|书包|钱包|腰包|行李箱|拉杆箱|羽绒服|外套|夹克|毛衣|卫衣|T恤|衬衫|裤|裙|袜|帽|围巾|手套|腰带|皮带|领带|领结|内衣|泳装|玩具|积木|拼图|遥控车|玩偶|公仔|乐高|魔方|陀螺|乐器|吉他|钢琴|电子琴|架子鼓|球|球拍|篮球|足球|羽毛球|网球|乒乓球|瑜伽垫|哑铃|杠铃|跳绳|跑步机|椭圆机|划船机|帐篷|睡袋|登山|徒步|滑雪|游泳圈|救生衣|自行车|平衡车|滑板|头盔|护膝|扳手|螺丝刀|锤子|钳子|电钻|电锯|角磨机|电锤|卷尺|水平仪|游标卡尺|万用表|灯泡|LED|开关|插座|电线|水管|阀门|龙头|花洒|地漏|瓷砖|地砖|地板|木板|涂料|油漆|乳胶漆|血压计|血糖仪|体温计|电子秤|轮椅|拐杖|助行器|按摩|理疗|牵引|护腰|护膝|护颈|婴儿车|推车|学步车|餐椅|婴儿床|摇篮|安全座椅/.test(n);
    return durKeywords.test(n);
  };
