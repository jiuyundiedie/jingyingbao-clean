...
const StockManage = () => {
  ...
  // 根据商品名称关键词自动推荐保质期天数（0表示不推荐）
  // 策略：最具体优先 → 冲突词排除 → 大类兜底
  // 按匹配风险/冲突概率排序：医疗/仪器 > 耐用品 > 食品
  const suggestExpiryByProductName = (name) => {
    ...
    // 医疗器械 / 康复 / 检测（最优先，避免"鱼跃"被"鱼"匹配）
    if (/电子血压计|水银血压计|血糖仪|血糖试纸|体温计|体重秤|电子秤|体脂秤|轮椅|拐杖|助行器|按摩椅|按摩器|颈椎牵引|腰椎牵引|护腰|护膝|护颈|疝气带|血压计|血糖仪/.test(n)) return 3650;

    // 药品 / 处方药 / 非处方药
    if (/阿莫西林|头孢|布洛芬|对乙酰氨基酚|阿司匹林|甲硝唑|奥美拉唑|感冒灵|感冒清热|止咳|化痰|润喉|创可贴|纱布块|碘伏|酒精棉球|医用口罩|口罩|体温计|维生素[0-9]|维生素C|维生素E|钙片|液体钙/.test(n)) return 730;

    // 母婴用品（先匹配，避免"纸尿裤"被"裤"匹配、"奶瓶"被"瓶"匹配）
    if (/纸尿裤|尿不湿|拉拉裤|学步裤|婴儿车|推车|学步车|餐椅|婴儿床|摇篮|安全座椅|奶瓶刷|奶瓶架|奶瓶|婴儿奶瓶|儿童霜|儿童牙刷|婴儿洗衣液/.test(n)) return 3650;
    // 母婴食品 → 540d
    if (/奶粉|婴儿奶粉|宝宝奶粉|配方奶粉|辅食米粉|宝宝辅食|果泥|肉泥/.test(n)) return 540;

    // ================= 第二梯队：耐用品（不会有保质期概念，给"建议更换周期"） =================

    // 电子产品（放在家电前面，因为有些电脑带"笔记本"）
    if (/iPhone|iPad|iPod|MacBook|Mac mini|Mac Studio|iMac|Apple Watch|AirPods|AirTag|小米手机|华为手机|HUAWEI|Mate\d+|MatePad|Pura\d+|苹果|手机|平板|ipad|电脑|笔记本电脑|台式机|主机|显示器|显卡|主板|硬盘|内存条|SSD|键盘|鼠标|音箱|耳机|蓝牙|充电头|充电宝|数据线|路由器|交换机|摄像头|相机|摄像机|无人机|投影仪|智能手表|运动手环|台灯|移动电源/.test(n)) return 1095;

    // 家电
    if (/冰箱|洗衣机|空调|空调扇|电视|微波炉|烤箱|蒸箱|油烟机|燃气灶|热水器|即热|吸尘器|扫地机器人|空气炸锅|电饭煲|压力锅|高压锅|电磁炉|豆浆机|榨汁机|原汁机|破壁机|净水器|净化器|加湿器|电风扇|落地扇|暖风机|消毒柜|洗碗机|挂烫机|电熨斗|电吹风|吹风机|卷发棒|剃须刀|理发器|电推子/.test(n)) return 1825;

    // 家具 / 家居
    if (/床|床架|床垫|沙发|茶几|电视柜|鞋柜|床头柜|衣柜|书柜|书桌|餐桌|餐边柜|斗柜|电视柜|窗帘|地毯|装饰画|相框|花瓶|花盆|花架|多层置物架|鞋架|衣柜|酒柜|餐边柜/.test(n)) return 7300;

    // 餐具 / 厨具 / 不锈钢 / 陶瓷 / 骨瓷
    if (/不锈钢|陶瓷|骨瓷|珐琅|铸铁|不粘锅|炒锅|汤锅|蒸锅|平底锅|高压锅|奶锅|煮蛋器|电蒸锅|菜刀|水果刀|套装刀|砧板|案板|保鲜盒|密封罐|饭盒|保温桶|保温饭盒|保温杯|马克杯|玻璃杯|碗|盘|筷|勺|叉|西餐刀|刀具套装|餐具套装/.test(n)) return 3650;

    // 文具 / 办公用品
    if (/便利贴|便签|便签纸|便签本|文件夹|文件袋|档案盒|档案袋|订书机|订书针|起钉器|回形针|大头针|长尾夹|胶带座|胶带切割器|透明胶带|双面胶|固体胶|胶水|美工刀|裁纸刀|剪刀|尺子|直尺|三角板|量角器|圆规|记号笔|马克笔|荧光笔|白板笔|签字笔|中性笔|圆珠笔|铅笔|自动铅笔|钢笔|毛笔|墨水|墨汁|橡皮擦|考试橡皮|笔记本|日记本|手账本|作业本|草稿本|素描本|速写本|日历|台历|挂历|便签|笔筒|文件盒|资料册|名片盒|名片夹|文件架/.test(n)) return 3650;

    // 鞋包 / 服饰 / 配件
    if (/帆布鞋|运动鞋|跑步鞋|篮球鞋|足球鞋|皮鞋|商务鞋|休闲鞋|拖鞋|凉鞋|高跟鞋|平底鞋|靴|雪地靴|马丁靴|登山鞋|板鞋|拖鞋|人字拖|包头拖|双肩包|背包|书包|手提包|单肩包|挎包|斜挎|托特包|双肩|钱包|卡包|零钱包|钥匙包|腰包|胸包|拉杆箱|行李箱|旅行箱|登机箱|羽绒服|棉服|夹克|外套|卫衣|毛衣|针织衫|T恤|衬衫|牛仔裤|休闲裤|西裤|短裤|运动裤|裙|连衣裙|半身裙|短裙|长裙|棉袜|丝袜|连裤袜|帽|棒球帽|渔夫帽|礼帽|草帽|围巾|手套|腰带|皮带|领带|领结|内衣|内裤|文胸|胸罩|泳装|泳衣|潜水服|披肩|斗篷|风衣|大衣|西装|中山装|唐装|汉服|旗袍/.test(n)) return 1825;

    // 运动 / 户外 / 健身
    if (/篮球|足球|羽毛球|网球|乒乓球|排球|橄榄球|高尔夫球|瑜伽垫|哑铃|杠铃|壶铃|跳绳|呼啦圈|拉力器|俯卧撑架|仰卧板|健腹板|哑铃凳|史密斯|跑步机|椭圆机|划船机|动感单车|健身车|帐篷|睡袋|登山杖|徒步鞋|冲锋衣|冲锋裤|登山包|徒步包|露营灯|防潮垫|烧烤架|卡式炉|滑雪|单板|双板|雪鞋|雪镜|雪服|游泳圈|救生衣|浮板|自行车|山地车|公路车|平衡车|滑板|长板|滑板车|头盔|护膝|护肘|护掌/.test(n)) return 1825;

    // 玩具 / 乐器 / 儿童
    if (/玩具|积木|拼图|遥控车|遥控飞机|无人机|玩偶|公仔|芭比|乐高|魔方|魔尺|陀螺|泡泡枪|泡泡机|画板|彩色铅笔|彩泥|橡皮泥|超轻粘土|太空沙|科学实验|DIY手工|乐器|吉他|钢琴|电子琴|手风琴|口琴|架子鼓|尤克里里|古筝|二胡|笛子|唢呐/.test(n)) return 3650;

    // 五金 / 工具 / 建材
    if (/扳手|螺丝刀|十字改锥|一字改锥|锤子|榔头|钳子|尖嘴钳|斜口钳|电钻|冲击钻|电锯|角磨机|电锤|电动螺丝刀|卷尺|水平仪|游标卡尺|万用表|电笔|灯泡|LED|吸顶灯|面板灯|筒灯|射灯|开关|插座|墙插|插排|电线|电缆|网线|水管|PVC管|PPR管|阀门|龙头|花洒|地漏|瓷砖|地砖|地板|木地板|实木地板|复合地板|铝扣板|石膏板|涂料|油漆|乳胶漆|腻子粉|美缝剂|免钉胶|玻璃胶|结构胶|膨胀螺丝|钉枪/.test(n)) return 3650;

    // ================= 第三梯队：食品 / 快消（有明确保质期） =================

    // 日化 / 清洁 / 香薰
    if (/洗发水|洗发露|沐浴露|沐浴乳|香皂|手工皂|牙膏|漱口水|牙线|洗面奶|洁面乳|洁面啫喱|护手霜|润肤霜|身体乳|面霜|精华|精华液|精华素|爽肤水|化妆水|乳液|眼霜|唇膏|口红|唇釉|粉底|粉饼|蜜粉|遮瑕|眼影|眉笔|眼线笔|睫毛膏|卸妆|面膜|眼膜|发膜|护发素|洗衣凝珠|洗衣液|洗衣粉|肥皂|洗洁精|餐具净|消毒液|除菌液|洁厕灵|洁厕液|玻璃水|地板清洁|油烟净|抽纸|卷纸|纸巾|面巾纸|餐巾纸|湿巾|酒精湿巾|厨房纸|垃圾袋|保鲜袋|保鲜膜|密封袋|真空袋|香薰|蜡烛|香熏|驱蚊|蚊香片|蚊香|花露水/.test(n)) return 730;

    // 宠物用品（先匹配，避免"猫砂"里的"豆"被蔬果匹配）
    if (/猫粮|狗粮|宠物粮|猫砂|豆腐猫砂|膨润土猫砂|混合猫砂|狗绳|牵引绳|猫抓板|宠物窝|狗窝|猫窝|喂水器|饮水机|喂食器|自动喂食|项圈|宠物玩具|磨牙棒|洁齿骨|驱虫/.test(n)) return 365;

    // 保健品 / 营养品
    if (/保健品|营养补充|蛋白粉|乳清蛋白|胶原蛋白|阿胶糕|人参|鹿茸|燕窝|西洋参|蜂王浆|口服液|颗粒|冲剂|氨基酸|鱼油|DHA|叶黄素|蓝莓素|护肝片|护肝宝/.test(n)) return 540;

    // 饮料 / 瓶装水（先匹配"纯净水"/"矿泉水"，避免漏掉）
    if (/饮料|可乐|雪碧|果汁|橙汁|苹果汁|葡萄汁|菠萝汁|咖啡|即饮咖啡|奶茶|瓶装奶茶|矿泉水|纯净水|桶装水|苏打水|气泡水|能量饮料|运动饮料|电解质水|椰子水|椰汁|豆奶|植物奶/.test(n)) return 365;

    // 干货 / 调料 / 米面 / 油（重点：匹配"油"避免"鱼/牛"误触发）
    if (/食用调和油|花生油|菜籽油|大豆油|玉米油|葵花籽油|橄榄油|香油|芝麻油|辣椒油|花椒油|醋|米醋|陈醋|香醋|盐|精盐|糖|白糖|冰糖|红糖|黑糖|酱油|生抽|老抽|蚝油|豆瓣酱|豆酱|甜面酱|辣椒酱|剁椒|老干妈|米|大米|小米|糯米|糙米|黑米|紫米|面粉|小麦粉|高筋粉|低筋粉|玉米面|淀粉|木薯粉|红薯粉|粉丝|粉条|粉皮|挂面|方便面调料|泡面调料|干货|木耳|香菇|红枣|枸杞|桂圆|银耳|莲子|百合|核桃|杏仁|腰果|花生|芝麻|花椒|八角|大料|茴香|桂皮|干辣椒|年糕|糍粑/.test(n)) return 365;

    // 零食 / 预包装 / 腌制品
    if (/薯片|膨化|饼干|曲奇|威化|巧克力|糖果|水果糖|奶糖|软糖|硬糖|坚果|混合坚果|辣条|魔芋|方便|泡面|方便面|袋装|罐装|预包装|海苔|瓜子|南瓜子|花生|泡椒|腌|蜜饯|果脯|话梅|梅干|桃干|杏干|葡萄干|枣|红枣干|红枣片|冻干果干/.test(n)) return 180;

    // 蔬果 / 生鲜
    if (/苹果|香蕉|橙|橘|葡萄|西瓜|草莓|蓝莓|芒果|梨|桃|樱桃|车厘子|猕猴桃|火龙果|百香果|柠檬|柚子|菠萝|榴莲|木瓜|椰子|柿子|荔枝|龙眼|山竹|石榴|蓝莓|桑葚|杨梅|蓝莓|杨梅|蓝莓|莓|蔬菜|青菜|白菜|大白菜|娃娃菜|包菜|甘蓝|生菜|油麦菜|菠菜|芹菜|韭菜|葱|姜|蒜|辣椒|青椒|彩椒|蘑菇|香菇|金针菇|杏鲍菇|平菇|木耳|海带|紫菜|土豆|马铃薯|番茄|西红柿|黄瓜|丝瓜|苦瓜|冬瓜|南瓜|西蓝花|花菜|白萝卜|胡萝卜|豆角|四季豆|豇豆|豌豆|毛豆|蚕豆|荷兰豆/.test(n)) return 5;

    // 熟食 / 即食 / 现做（短期）
    if (/熟食|卤味|卤|烤鸭|烧鸡|白切鸡|盐水鸭|腊肉|腊鱼|腊味|香肠|红肠|午餐肉|凉菜|凉拌|即食|现做|快餐|便当|盒饭|三明治|寿司|沙拉|披萨|汉堡|炸鸡|薯条/.test(n)) return 2;

    // 面包 / 烘焙（短期）
    if (/面包|吐司|蛋糕|糕点|月饼|蛋挞|泡芙|饼干|酥|蛋黄派|派|千层|面包片|手撕面包|牛角包|法棍|欧包|全麦面包|戚风|海绵蛋糕/.test(n)) return 3;

    // 乳制品 / 冷鲜（短期，含鲜牛奶，避免和"饮料"冲突因为已经排前面了）
    if (/牛奶|鲜奶|巴氏奶|常温奶|酸奶|安慕希|畅轻|伊利|蒙牛|特仑苏|金典|奶酪|芝士|黄油|奶油|炼乳|淡奶油|冰淇淋|雪糕|冰棒|冰棍/.test(n)) return 7;

    // 生鲜 / 肉类 / 海鲜（短期）
    if (/生鲜|冷鲜|鲜肉|排骨|牛肉|羊肉|猪肉|鸡肉|鸭肉|鹅肉|兔肉|鱼肉|草鱼|鲤鱼|鲫鱼|鲈鱼|带鱼|虾|基围虾|明虾|河虾|对虾|大虾|蟹|大闸蟹|梭子蟹|贝类|蛤蜊|蚬|生蚝|扇贝|三文鱼|金枪鱼|切片|培根|火腿|肉肠/.test(n)) return 2;

    // 农资 / 园艺
    if (/肥料|化肥|复合肥|有机肥|营养液|花肥|农药|杀虫剂|除草剂|杀菌剂|种子|花种|菜种|除草剂|花盆|花架|喷壶|浇水壶|锄头|铁锹|耙子|镰刀|花卉|多肉|绿植|鲜花|玫瑰|月季|桂花|茉莉|绿萝|发财树/.test(n)) return 365;

    // 默认 → 90天（AI 识别新商品时会用 expiryHint 覆盖）
    return 90;
  };
...
返回字段：
{
  "name": "商品标准名称（品牌+规格+包装）",
  "brand": "品牌，无则空字符串",
  "category": "品类：乳制品/烘焙熟食/生鲜蔬果/零食饮料/干货调料/保健品/医药用品/日化清洁/母婴用品/电子产品/家电/餐具厨具/文具办公/家具家居/鞋包皮具服饰/玩具乐器/运动户外/五金建材/医疗器械/农资园艺/宠物用品/其他",
  "expiryHint": 保质期提示（天数，整数），耐用品填3650,
  "confidence": 识别置信度0-1,
  "count": 识别到的件数,
  "barcode": "条码数字，看到才填"
}`;
